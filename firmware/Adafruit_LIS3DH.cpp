...
/**************************************************************************/
/*!
    @brief  Sets motion interrupt mode

    @param movethreshold  number of least significant bits of movement to trigger
                          for example, if the accelerometer is on 2G mode, each
                          LSB is 15.625mg, so a value of 32 would be 500mg of movement
    #param mintime        number of LSB of time to trigger
                          for example, if the accelerometer is set to 25Hz sampling,
                          each LSB would be 1/25 second, so a value of 10 would be
                          40 milliseconds
*/
/**************************************************************************/
void Adafruit_LIS3DH::setInterrupt(uint8_t movethreshold, uint8_t mintime)
{
  writeRegister8(LIS3DH_REG_CTRL3, 0x40);   // enable interrupt 1 AOI
  writeRegister8(LIS3DH_REG_CTRL5, 0x0C);   // latch interrupt, 4d enable (?)
  writeRegister8(LIS3DH_REG_INT1THS, movethreshold);   // set move threshold
  writeRegister8(LIS3DH_REG_INT1DUR, mintime);        // set minimum duration
  writeRegister8(LIS3DH_REG_INT1CFG, 0x3F);           // OR combination of any axes
}

/**************************************************************************/
/*!
    @brief  Clears a latched motion interrupt
*/
/**************************************************************************/
void Adafruit_LIS3DH::clearInterrupt()
{
